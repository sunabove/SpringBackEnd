...
public class Ex120_StringFormatExample {
    public static void main(String[] args) {
        ...
        formattedDouble = String.format("Pi=%10.4f", pi); 
        System.out.println( "[" + formattedDouble + "]" ); // 출력: Pi: 3.14
        ...
        // 4. 자리 맞춤 포매팅
        int value = 123;
        String paddedValue = String.format( "[%10d]", value ); // 5자리 확보하고 우측 정렬
        System.out.println(paddedValue); // 출력: '  123'

        // 5. 왼쪽 정렬과 오른쪽 정렬
        String leftAligned = String.format("[%-20s]", name);  // 왼쪽 정렬
        System.out.println(leftAligned); // 출력: 'Alice     '

        String rightAligned = String.format("[%20s]", name);  // 오른쪽 정렬
        System.out.println(rightAligned); // 출력: '     Alice'

        // 6. 16진수로 포매팅
        int hexNumber = 255;
        String hexFormatted = String.format("16진수: %x", hexNumber);
        System.out.println( hexNumber ); // 출력: 16진수: ff
        System.out.println(hexFormatted); // 출력: 16진수: ff
        String hexFormattedUpper = String.format("16진수: %X", hexNumber);
        System.out.println(hexFormattedUpper); // 출력: 16진수: ff
